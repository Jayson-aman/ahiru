// v1.2.0 マーケティングスクリーンショット 第2弾（1242×2688px = Apple 6.5"標準サイズ）
// ホーム画面の資格タイル（20資格ぶん）・問題・教科書・図解・模試が伝わるよう10枚構成。
// 新旧の資格をバランスよく含める。
// 使い方: node cap_v3.js            → 全10枚
//         node cap_v3.js 03 07      → 指定した番号だけ撮り直す
const { chromium } = require('./_playwright');
const fs = require('fs');

const BASE = 'http://127.0.0.1:8899';
const OUT = '/home/user/ahiru/store/screenshots_v3';
...
(async () => {
  ...
  if (want('01')) {
    console.log('01 ホーム（資格一覧・上部：英検/宅建/マン管/FP/二級建築士）');
    await go('/');
  ...
  if (want('05')) {
    console.log('05 マンション管理士 図解つき解説');
    await answer(page, '/mansion/kubunsho', { figure: true, correct: true });
    await scrollToText(page, '図解', -300);
    await shot(page, '05-mansion-figure');
  }

  if (want('06')) {
    console.log('06 建築設備士 要点テキスト本文');
    await go('/kenchiku/text/ippan', 3200);
    await shot(page, '06-kenchiku-text');
  }

  if (want('07')) {
    console.log('07 危険物乙4 図解つき解説');
    await answer(page, '/kikenbutsu/horei', { figure: true, correct: true });
    await scrollToText(page, '図解', -300);
    await shot(page, '07-kikenbutsu-figure');
  }

  if (want('08')) {
    console.log('08 二級建築士 図解つき解説');
    await answer(page, '/nikkyu/keikaku', { figure: true, correct: true });
    await scrollToText(page, '図解', -300);
    await shot(page, '08-nikkyu-figure');
  }

  if (want('09')) {
    console.log('09 第二種電気工事士 要点テキスト（複線図の描き方を含む）');
    await go('/denki2/text/riron', 3200);
    await shot(page, '09-denki2-text');
  }

  if (want('10')) {
    console.log('10 給水装置工事主任技術者 模擬試験（タイマー付き本試験形式）');
    await go('/kyusui/mogi', 2200);
    await shot(page, '10-kyusui-mogi');
  }
