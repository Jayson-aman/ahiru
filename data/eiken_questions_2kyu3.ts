...
export const eikenQuestions2kyu3: EikenQuestion[] = [
  ...
  // 008 — vocabulary
  {
    ...
    choices: [
      { key: 'A', text: 'escalate' },
      { key: 'B', text: 'fabricate' },
      { key: 'C', text: 'alleviate' },
      { key: 'D', text: 'provoke' },
    ],
    correctKey: 'C' as const,
  ...
  // 031 — No sooner ... than
  {
    ...
    choices: [
      ...
      { key: 'B', text: 'had she entered' },
      { key: 'C', text: 'she had entered' },
      { key: 'D', text: 'has she entered' },
    ],
    correctKey: 'B' as const,
  ...
  // 032 — Present perfect vs. simple past
  {
    ...
    choices: [
      { key: 'A', text: 'experienced' },
      { key: 'B', text: 'has experienced' },
  ...
  // 035 — Perfect infinitive with modal
  {
    ...
    choices: [
      ...
      { key: 'B', text: 'should have consulted' },
      { key: 'C', text: 'must consult' },
      { key: 'D', text: 'ought to consult' },
    ],
    correctKey: 'B' as const,
  ...
  // --- PASSAGE 1: Science/Technology (AI & Medicine) — Q036-038 ---
  {
    ...
    choices: [
      ...
      { key: 'B', text: 'The decision-making process of AI is difficult to understand even for its creators.' },
      { key: 'C', text: 'AI requires too much electricity to be practical in hospitals.' },
      { key: 'D', text: 'Medical AI must be kept in secure, isolated computer servers.' },
    ],
    correctKey: 'B' as const,
    explanation: 'パッセージでは「an AI\'s logic may be opaque even to its developers—a phenomenon known as the \'black box\' problem」と説明されています。AIの判断プロセスが開発者にさえ不透明であることを指します。Bがこれを正確に言い換えています。',
    difficulty: 'standard' as const,
  },
...

  {
    ...
    choices: [
      ...
      { key: 'B', text: 'The impact of globalization on food culture is complex and varies by context.' },
      { key: 'C', text: 'Traditional food cultures are strong enough to resist globalization without any consequences.' },
      { key: 'D', text: 'Rural communities are better at preserving food traditions than urban areas.' },
    ],
    correctKey: 'B' as const,
    explanation: 'パッセージは「The situation is more complex than it first appears」と明言し、都市部では地元料理への関心が高まる一方、農村部では課題が続くという複雑な状況を描いています。Bが「文脈によって影響が異なる」という全体的な主張を最も正確に表しています。',
    difficulty: 'advanced' as const,
  },
  ...
  // --- PASSAGE 3: Environmental Topic (Ocean Pollution) — Q042-044 ---
  {
    ...
    choices: [
      ...
      { key: 'B', text: 'Stopping plastic before it reaches the ocean prevents further dispersion.' },
      { key: 'C', text: 'River interception is cheaper because it requires fewer workers.' },
      { key: 'D', text: 'Ocean cleanup removes only large pieces of plastic, not microplastics.' },
    ],
    correctKey: 'B' as const,
    explanation: 'パッセージは「interceptor devices in rivers to catch debris before it reaches the sea」が「post-pollution ocean cleanup operations」より効果的だとしています。海に入ってしまった後のクリーンアップより、入る前に止める方が分散を防げるという論理です。Bがこれを最もよく表しています。',
    difficulty: 'standard' as const,
  },
