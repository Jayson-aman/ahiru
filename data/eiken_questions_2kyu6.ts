...
export const eikenQuestions2kyu6: EikenQuestion[] = [
  ...
  },
  {
    ...
    correctKey: 'A',
    explanation: 'Whether 〜（〜かどうかということ）の名詞節が文の主語になる形が正解。「明日ハイキングに行くかどうかは天気次第だ」の意味です。ifも「〜かどうか」を表せますが、if節は文の主語としては使えないのがルールです。What・Whichでは節の中の要素が欠けていないため文法的に成り立ちません。whether と if はどちらも「〜かどうか」を表すが、文中で果たせる役割が異なるため、節の位置を確認してから選ぶ。\n\n【図解】whether と if の違い\n　用法｜whether｜if\n　主語になる節｜可｜不可\n　目的語になる節｜可｜可\n　or not を直後に置く｜可｜不可\n　⇒ 文の主語になるので Whether\n　⚠ if は目的語の位置でしか使えない。節の役割で使い分ける。',
    difficulty: 'advanced',
  },
  {
  ...
  },
  {
    ...
    correctKey: 'B',
    explanation: 'either A or B（AかBのどちらか）の構文が正解。「メインコースには肉料理か魚料理のどちらかを選べます」の意味で、後ろのorと呼応するのはeitherです。neitherはnorと、bothはandと呼応するため、orとは組み合わせられません。allは3つ以上のものに使います。相関接続詞は呼応する語の組み合わせが固定されているので、片方が見えればもう片方は自動的に決まる。\n\n【図解】相関接続詞の呼応\n　語｜呼応する語｜数の扱い\n　either｜or｜Bに一致\n　neither｜nor｜Bに一致\n　both｜and｜複数扱い\n　⇒ 後ろに or があるので either\n　⚠ 呼応する語の組み合わせは固定。either 〜 nor のような混用はできない。',
    difficulty: 'basic',
  },
  {
  ...
  },
  {
    ...
    correctKey: 'A',
    explanation: '形式主語のItが正解。It is ... to do（〜することは…だ）の構文で、真の主語はto check以下です。「牛乳を買う前に賞味期限を確認することが大切だ」の意味です。There is構文は存在を表す別の構文で、ThatやWhatをこの位置に置くとto不定詞とつながりません。形式主語の It は後ろに置いた to 不定詞や that 節を受ける働きをし、長い主語を後ろに回して読みやすくする。\n\n【図解】形式主語の構文\n　構文｜真主語｜例\n　It is 形容詞 to do｜to 不定詞｜It is important to check\n　It is 形容詞 that 節｜that 節｜It is clear that\n　There is 構文｜存在を表す｜There is a book\n　⇒ to 不定詞が続くので It\n　⚠ There is は存在文で形式主語ではない。構文の働きを取り違えない。',
    difficulty: 'basic',
  },
  {
...
In the passage, the word "grounds" most closely means:`,
    ...
    correctKey: 'D',
    explanation: 'groundは動詞で「（航空機などを）飛べなくする・地上にとどめる」という意味があり、Dが正解。「悪天候になるとドローンは簡単に飛行できなくなる」という文脈です。名詞の「地面」の意味から派生した用法で、飛行機の欠航のニュースなどでも使われます。A・B・Cはいずれも文脈に合いません。ground は名詞の「地面」から派生して、航空機を地上にとどめるという他動詞の用法を持つようになった語である。\n\n【図解】ground の品詞と意味\n　品詞｜意味｜例\n　動詞｜飛行を差し止める｜flights were grounded\n　名詞｜地面, 土地｜on the ground\n　名詞（複数）｜根拠｜on the grounds that\n　⇒ 悪天候で飛べなくなるなら grounded\n　⚠ 名詞の「地面」の意味で読むと文意が通らない。動詞用法を押さえる。',
    difficulty: 'advanced',
  },
  {
...
Which benefit of green roofs is mentioned in the passage?`,
    ...
    correctKey: 'C',
    explanation: '本文に「緑化屋根は雨水を吸収して洪水を減らし、土と植物の層が建物を断熱して冷暖房費を削減する」とあり、Cが正解。Dは排水システムが不要になるとまでは述べられておらず、A・Bは本文に記述がありません。多数の緑化屋根が地区全体の気温を下げるヒートアイランド緩和効果も挙げられています。設問は緑化屋根の効果を問うており、保水による洪水対策と断熱による省エネという二つの側面が本文で並列に示されている。\n\n【図解】緑化屋根の効果\n　効果｜仕組み｜結果\n　洪水の抑制｜雨水を吸収する｜排水負荷の軽減\n　断熱｜土と植物の層｜冷暖房費の削減\n　都市の冷却｜多数設置で地区全体｜ヒートアイランド緩和\n　⇒ 保水と断熱が主な効果\n　⚠ 「排水設備が不要になる」とまでは述べていない。効果の程度を誇張しない。',
    difficulty: 'standard',
  },
  {
...
According to the passage, how does music help athletes?`,
    ...
    correctKey: 'B',
    explanation: '本文に「力強く安定したビートの音楽は、規則的なリズムを保つのを助け、疲労感から注意をそらし、持久力を小幅ながら測定可能な程度に向上させる」とあり、Bが正解。効果が最も大きいのはランニングや自転車など動きをテンポに合わせられる反復運動だとも述べられています。A・C・Dは本文にありません。設問は運動中に音楽が果たす役割を問うており、効果が小幅であると明記されている点まで含めて読み取る必要がある。\n\n【図解】音楽が運動に及ぼす効果\n　要素｜本文の記述｜効果\n　安定したビート｜規則的なリズムの維持｜動きが整う\n　注意の転換｜疲労感からそらす｜つらさが減る\n　持久力｜小幅だが測定可能な向上｜数値で確認できる\n　⇒ リズム維持と注意の転換が主な効果\n　⚠ 効果は「小幅」と明記されている。程度を過大に読み替えない。',
    difficulty: 'standard',
  },
  {
...
In the passage, the word "glowing" most closely means:`,
    ...
    correctKey: 'B',
    explanation: 'glowingは「絶賛の・べたぼめの」という意味で、Bが正解。自社製品への絶賛レビューを書かせ、競合には辛辣なレビューを書かせる業者がいる、という文脈です。直後のharsh ones（辛辣なレビュー）との対比からも判断できます。「光り輝く」という文字どおりの意味から比喩的に発展した用法です。glowing は本来「輝いている」を意味する語で、そこから「絶賛の」という比喩的な用法が生まれた。\n\n【図解】glowing の意味\n　語｜意味｜文脈\n　glowing｜絶賛の｜glowing reviews\n　harsh｜辛辣な｜対比される語\n　原義｜光り輝く｜比喩的に発展\n　⇒ harsh ones と対比されるなら絶賛の意味\n　⚠ 文字どおり「光る」と読むと文意が通らない。対比表現から意味を推定する。',
    difficulty: 'standard',
  },
  {
...
What do consumer groups advise readers of online reviews to do?`,
    ...
    correctKey: 'A',
    explanation: '本文最終文に「極端な意見には懐疑的になり、良い点と悪い点の両方に触れた詳細なレビューに最も注意を払い、レビュアーにバランスの取れたコメントの履歴があるか確認するよう助言している」とあり、Aが正解。Bの五つ星だけを信じる行動はむしろ危険とされ、C・Dは本文にありません。設問はレビューを見極める助言の内容を問うており、星の数だけでなく記述の詳しさと投稿者の履歴が判断材料になる。\n\n【図解】レビューの見極め方\n　助言｜本文の記述｜理由\n　極端な意見に懐疑的になる｜be skeptical｜偏りの可能性\n　詳細なレビューを重視｜良い点と悪い点の両方｜バランス\n　投稿者の履歴を確認｜balanced comments｜信頼性の判断\n　⇒ 詳細でバランスのとれた評価を重視する\n　⚠ 五つ星だけを見る行動は危険とされる。助言の内容を正確に読む。',
    difficulty: 'standard',
  },
  {
