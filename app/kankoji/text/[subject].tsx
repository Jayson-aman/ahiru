...
import { getKankojiText } from '../../../data/kankoji_text';
import StudyText from '../../../components/StudyText';

const INFO: Record<string, { name: string; emoji: string; color: string }> = {
  genron: { name: '原論・電気・建築', emoji: '🌡️', color: '#455A64' },
  kucho: { name: '空調設備', emoji: '❄️', color: '#1565C0' },
  eisei: { name: '給排水衛生設備', emoji: '🚰', color: '#00838F' },
  sekou: { name: '施工管理法', emoji: '🔧', color: '#E65100' },
  hoki: { name: '法規', emoji: '⚖️', color: '#6A1B9A' },
};

export default function KankojiTextScreen() {
  const { subject } = useLocalSearchParams<{ subject: string }>();
  const router = useRouter();
  const info = INFO[subject ?? ''] ?? { name: subject, emoji: '📖', color: '#00695C' };
  const sections = getKankojiText(subject ?? '');
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={[styles.tipBox, { borderLeftColor: info.color }]}>
          ...
          <Text style={styles.tipText}>
            テキストで頻出数値と原理を押さえたら、科目別問題で知識を定着させましょう。★マークは本試験で特に狙われる論点です。
          </Text>
        </View>
      </ScrollView>
    </SafeAreaView>
