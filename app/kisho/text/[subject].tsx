...
const INFO: Record<string, { name: string; emoji: string; color: string }> = {
  gakka1: { name: '一般知識', emoji: '🌡️', color: '#1565C0' },
  gakka2: { name: '専門知識', emoji: '🌀', color: '#0097A7' },
};

export default function KishoTextScreen() {
  const { subject } = useLocalSearchParams<{ subject: string }>();
  const router = useRouter();
  const info = INFO[subject ?? ''] ?? { name: subject, emoji: '📖', color: '#1565C0' };
  const sections = kishoTextbook
              ...
              <ScrollView contentContainerStyle={styles.content}>
                ...
                <View style={[styles.tipBox, { borderLeftColor: info.color }]}>
                  ...
                  <Text style={styles.tipText}>
                    テキストで基礎を固めたら分野別問題で知識を定着させ、実技試験で天気図の読み方・記述の型を仕上げましょう。
                  </Text>
                </View>
              </ScrollView>
            </>
