...
import { getTakkeiText } from '../../../data/takkei_text';
...
const INFO: Record<string, { name: string; emoji: string; color: string }> = {
  kenri: { name: '権利関係', emoji: '🤝', color: '#1565C0' },
  gyoho: { name: '宅建業法', emoji: '📋', color: '#C62828' },
  horei: { name: '法令上の制限', emoji: '🏙️', color: '#2E7D32' },
  zei: { name: '税・その他', emoji: '🧾', color: '#E65100' },
};

export default function TakkeiTextScreen() {
  const { subject } = useLocalSearchParams<{ subject: string }>();
  const router = useRouter();
  const info = INFO[subject ?? ''] ?? { name: subject, emoji: '📖', color: '#BF360C' };
  const sections = getTakkeiText(subject ?? '');
              ...
              <ScrollView contentContainerStyle={styles.content}>
                ...
                <View style={[styles.tipBox, { borderLeftColor: info.color }]}>
                  ...
                  <Text style={styles.tipText}>
                    テキストで基礎を固めたら分野別問題で知識を定着させ、本試験問題（過去5年分）で実戦力を仕上げましょう。個数問題・統計問題も収録しています。
                  </Text>
                </View>
              </ScrollView>
            </>
