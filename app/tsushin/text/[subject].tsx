...
import { getTsushinText } from '../../../data/tsushin_text';
...
const INFO: Record<string, { name: string; emoji: string; color: string }> = {
  kogaku:  { name: '電気通信工学',   emoji: '📡', color: '#1565C0' },
  setsubi: { name: '電気通信設備',   emoji: '🌐', color: '#00695C' },
  kanri:   { name: '施工管理法',     emoji: '📅', color: '#EF6C00' },
  hoki:    { name: '法規',           emoji: '⚖️', color: '#6A1B9A' },
};

export default function TsushinTextScreen() {
  const { subject } = useLocalSearchParams<{ subject: string }>();
  const router = useRouter();
  const info = INFO[subject ?? ''] ?? { name: subject, emoji: '📖', color: '#1565C0' };
  const sections = getTsushinText(subject ?? '');
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <CertPaywall
        certKey="tsushin"
        certName={`電気通信施工 ${info.name} テキスト`}
              ...
              <ScrollView contentContainerStyle={styles.content}>
                ...
                <View style={[styles.tipBox, { borderLeftColor: info.color }]}>
                  ...
                  <Text style={styles.tipText}>
                    テキストで基礎を固めたら科目別問題で知識を定着させ、模擬試験で仕上げましょう。一次検定は四肢択一で60%が合格ライン。計算問題（dB・伝送速度・工程表）は毎年出るので必ず押さえましょう。
                  </Text>
                </View>
              </ScrollView>
            </>
