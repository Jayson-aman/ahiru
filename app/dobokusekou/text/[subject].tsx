...
const INFO: Record<string, { name: string; emoji: string; color: string }> = {
  ...
  senmon: { name: '専門土木', emoji: '🌊', color: '#00695C' },
  kanri: { name: '施工管理法', emoji: '📅', color: '#2E7D32' },
  hoki: { name: '土木法規', emoji: '⚖️', color: '#6A1B9A' },
};

export default function DobokusekouTextScreen() {
  const { subject } = useLocalSearchParams<{ subject: string }>();
  const router = useRouter();
  const info = INFO[subject ?? ''] ?? { name: subject, emoji: '📖', color: '#00695C' };
  const sections = getDobokusekouText(subject ?? '');
