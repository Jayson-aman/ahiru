...
import { getMansionText } from '../../../data/mansion_text';
...
export default function MansionTextScreen() {
  const { subject } = useLocalSearchParams<{ subject: string }>();
  const router = useRouter();
  const info = INFO[subject ?? ''] ?? { name: subject, emoji: '📖', color: '#4A0072' };
  const sections = getMansionText(subject ?? '');
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={[styles.tipBox, { borderLeftColor: info.color }]}>
          ...
          <Text style={styles.tipText}>
            テキストで決議要件・数値基準を押さえたら、分野別問題で定着させ、本試験問題（令和6・7年度）で実戦力を仕上げましょう。★マークは特に頻出の論点です。
          </Text>
        </View>
      </ScrollView>
    </SafeAreaView>
