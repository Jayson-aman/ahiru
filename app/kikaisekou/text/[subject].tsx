...
import { getKikaisekouText } from '../../../data/kikaisekou_text';
...
export default function KikaisekouTextScreen() {
  const { subject } = useLocalSearchParams<{ subject: string }>();
  const router = useRouter();
  const info = INFO[subject ?? ''] ?? { name: subject, emoji: '📖', color: '#546E7A' };
  const sections = getKikaisekouText(subject ?? '');
