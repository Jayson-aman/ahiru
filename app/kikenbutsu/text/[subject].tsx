...
import { getKikenbutsuText } from '../../../data/kikenbutsu_text';
...
const INFO: Record<string, { name: string; emoji: string; color: string }> = {
  horei:     { name: '危険物に関する法令', emoji: '📜', color: '#C62828' },
  butsuri:   { name: '物理学・化学',       emoji: '🧪', color: '#1565C0' },
  seishitsu: { name: '危険物の性質と消火', emoji: '⛽', color: '#EF6C00' },
};

export default function KikenbutsuTextScreen() {
  const { subject } = useLocalSearchParams<{ subject: string }>();
  const router = useRouter();
  const info = INFO[subject ?? ''] ?? { name: subject, emoji: '📖', color: '#C62828' };
  const sections = getKikenbutsuText(subject ?? '');
              ...
              <ScrollView contentContainerStyle={styles.content}>
                ...
                <View style={[styles.tipBox, { borderLeftColor: info.color }]}>
                  ...
                  <Text style={styles.tipText}>
                    テキストで基礎を固めたら科目別問題で知識を定着させ、模擬試験で仕上げましょう。3科目それぞれ60%以上で合格です。1科目でも60%を切ると不合格になるため、苦手科目を作らないことが鍵です。
                  </Text>
                </View>
              </ScrollView>
            </>
