...
const SUBJECTS = [
  { key: 'kenchikugaku', name: '建築学等', emoji: '📐', color: '#1565C0', desc: '環境工学・構造力学・建築材料・共通（設備・契約）' },
  { key: 'sekou', name: '躯体・仕上施工', emoji: '🏗️', color: '#E65100', desc: '土工事・鉄筋・型枠・コンクリート・鉄骨・防水・内装' },
  { key: 'kanri', name: '施工管理法', emoji: '📋', color: '#2E7D32', desc: '施工計画・工程管理・品質管理・安全管理' },
  { key: 'hoki', name: '法規', emoji: '⚖️', color: '#7B1FA2', desc: '建設業法・建築基準法・労働安全衛生法・環境関連法' },
];

export default function SekokanTextIndexScreen() {
  const router = useRouter();
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        <TouchableOpacity onPress={() => router.back()}><Text style={styles.back}>← 戻る</Text></TouchableOpacity>
        <Text style={styles.title}>📖 一級建築施工管理技士テキスト</Text>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        {SUBJECTS.map(s => (
          <TouchableOpacity
            key={s.key}
            style={styles.card}
            onPress={() => router.push(`/sekokan/text/${s.key}` as any)}
            activeOpacity={0.85}
          >
