...
export default function SekokanTextScreen() {
  const { subject } = useLocalSearchParams<{ subject: string }>();
  const router = useRouter();
  const info = INFO[subject ?? ''] ?? { name: subject, emoji: '📖', color: '#BF360C' };
  const sections = getSekokanText(subject ?? '');
          ...
          <ScrollView contentContainerStyle={styles.content}>
            ...
            <View style={[styles.tipBox, { borderLeftColor: info.color }]}>
              ...
              <Text style={styles.tipText}>
                テキストで基礎を固めたら「基礎問題」で定着を確認し、「応用問題」で本試験レベルの出題（応用能力問題水準）に挑戦しましょう。仕上げは「模擬試験」で時間配分の練習を。二次検定の経験記述対策は「第二次検定 対策ガイド」を参照してください。
              </Text>
            </View>
          </ScrollView>
          </>
