...
const INFO: Record<string, { name: string; emoji: string; color: string }> = {
  keikaku: { name: '建築計画',   emoji: '📐', color: '#00695C' },
  hoki:    { name: '建築法規',   emoji: '⚖️', color: '#6A1B9A' },
  kozo:    { name: '建築構造',   emoji: '🏛️', color: '#1565C0' },
  sekou:   { name: '建築施工',   emoji: '🏗️', color: '#EF6C00' },
};

export default function NikkyuTextScreen() {
  const { subject } = useLocalSearchParams<{ subject: string }>();
  const router = useRouter();
  const info = INFO[subject ?? ''] ?? { name: subject, emoji: '📖', color: '#00695C' };
  const sections = getNikkyuText(subject ?? '');
              ...
              <ScrollView contentContainerStyle={styles.content}>
                ...
                <View style={[styles.tipBox, { borderLeftColor: info.color }]}>
                  ...
                  <Text style={styles.tipText}>
                    テキストで基礎を固めたら科目別問題で知識を定着させ、模擬試験で仕上げましょう。学科は4科目各25問の計100問。各科目に足切り点があるため、苦手科目を作らないことが合格の鍵です。
                  </Text>
                </View>
              </ScrollView>
            </>
