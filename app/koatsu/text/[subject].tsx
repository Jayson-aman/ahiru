...
const INFO: Record<string, { name: string; emoji: string; color: string }> = {
  hoan:         { name: '保安管理技術（共通）', emoji: '🛡️', color: '#37474F' },
  horei:        { name: '法令（共通）',        emoji: '📜', color: '#6A1B9A' },
  gakushiki_ka: { name: '学識（化学）',        emoji: '🧪', color: '#1565C0' },
  gakushiki_ki: { name: '学識（機械）',        emoji: '⚙️', color: '#C62828' },
};

export default function KoatsuTextScreen() {
  const { subject } = useLocalSearchParams<{ subject: string }>();
  const router = useRouter();
  const info = INFO[subject ?? ''] ?? { name: subject, emoji: '📖', color: '#37474F' };
  const sections = getKoatsuText(subject ?? '');
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <CertPaywall
        certKey="koatsu"
        certName={`高圧ガス製造保安責任者 ${info.name} テキスト`}
          ...
          return (
            <>
              <View style={[styles.header, { backgroundColor: info.color }]}>
                ...
                <Text style={styles.subTitle}>{sections.length}セクション収録</Text>
              </View>
              <ScrollView contentContainerStyle={styles.content}>
                ...
                <View style={[styles.tipBox, { borderLeftColor: info.color }]}>
                  ...
                  <Text style={styles.tipText}>
                    「保安管理技術」「法令」は甲種・乙種・丙種を問わず共通で問われる科目です。まずここを固め、志望する区分（化学・機械）の学識を重点的に学習しましょう。
                  </Text>
                </View>
              </ScrollView>
            </>
