...
export default function KoatsuQuizScreen() {
  const { subject } = useLocalSearchParams<{ subject: string }>();
  const router = useRouter();
  const info = SUBJECT_INFO[subject ?? ''] ?? { name: subject, emoji: '📖', color: '#37474F' };

  const allQuestions = koatsuQuestions
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <CertPaywall
        certKey="koatsu"
        certName={`高圧ガス製造保安責任者 ${info.name}`}
