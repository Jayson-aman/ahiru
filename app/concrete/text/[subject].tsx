...
const INFO: Record<string, { name: string; emoji: string; color: string }> = {
  zairyo:  { name: 'コンクリート用材料', emoji: '🧱', color: '#6D4C41' },
  haigo:   { name: '配合設計と性質',     emoji: '📐', color: '#1565C0' },
  seizo:   { name: '製造・品質管理',     emoji: '🏭', color: '#00695C' },
  sekou:   { name: '施工',               emoji: '🏗️', color: '#EF6C00' },
  kakushu: { name: '各種コンクリート',   emoji: '🧊', color: '#5E35B1' },
  shindan: { name: '劣化診断・補修補強', emoji: '🔍', color: '#C62828' },
};

export default function ConcreteTextScreen() {
  const { subject } = useLocalSearchParams<{ subject: string }>();
  const router = useRouter();
  const info = INFO[subject ?? ''] ?? { name: subject, emoji: '📖', color: '#6D4C41' };
  const sections = getConcreteText(subject ?? '');
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <CertPaywall
        certKey="concrete"
        certName={`コンクリート技士 ${info.name} テキスト`}
              ...
              <ScrollView contentContainerStyle={styles.content}>
                ...
                <View style={[styles.tipBox, { borderLeftColor: info.color }]}>
                  ...
                  <Text style={styles.tipText}>
                    テキストで基礎を固めたら科目別問題で知識を定着させ、模擬試験で仕上げましょう。技士は四肢択一40問＋小論文、診断士は四肢択一40問＋記述式です。配合計算と劣化機構は毎年出るので必ず押さえましょう。
                  </Text>
                </View>
              </ScrollView>
            </>
