...
export default function QuizScreen() {
  ...
  const saveKey = difficultyFilter ? `${subjectKey}_${difficultyFilter}` : subjectKey;
  ...
  async function handleAnswer(correct: boolean) {
    ...
    if (currentIndex + 1 >= total) {
      if (!savedProgress) {
        ...
        await saveProgress(saveKey, newScore, total);
      }
      setScore(newScore);
      setFinished(true);
    } else {
    ...
    return (
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyText}>この難易度の問題はまだありません</Text>
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={[styles.header, { backgroundColor: info.color }]}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.headerTitle}>{info.name}</Text>
          {diffInfo != null && (
            <Text style={styles.headerDiff}>{diffInfo.icon}</Text>
          )}
        </View>
        <View style={styles.headerRight}>
          <TouchableOpacity
      ...
      >
        <View style={styles.scoreRow}>
          ...
          {diffInfo != null && (
            <View style={[styles.diffFilterBadge, { borderColor: diffInfo.color }]}>
              <Text style={[styles.diffFilterText, { color: diffInfo.color }]}>
                {diffInfo.icon} {diffInfo.label}
              </Text>
            </View>
          )}
