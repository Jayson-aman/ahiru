...
const INFO: Record<string, { name: string; emoji: string; color: string }> = {
  kiso:    { name: '基礎的な物理学的性質及び化学的性質', emoji: '🧪', color: '#1565C0' },
  nensho:  { name: '燃焼工学に関する知識',           emoji: '🔥', color: '#C62828' },
  kyokyu:  { name: '供給設備に関する知識',           emoji: '🚚', color: '#00838F' },
  shohi:   { name: '消費設備に関する知識',           emoji: '🏠', color: '#EF6C00' },
  horei:   { name: '関係法令',                      emoji: '📜', color: '#6A1B9A' },
};

export default function LpgasTextScreen() {
  const { subject } = useLocalSearchParams<{ subject: string }>();
  const router = useRouter();
  const info = INFO[subject ?? ''] ?? { name: subject, emoji: '📖', color: '#F57F17' };
  const sections = getLpgasText(subject ?? '');
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <CertPaywall
        certKey="lpgas"
        certName={`液化石油ガス設備士 ${info.name} テキスト`}
          ...
          return (
            <>
              <View style={[styles.header, { backgroundColor: info.color }]}>
                ...
                <Text style={styles.subTitle}>学科試験レベル ／ {sections.length}セクション収録</Text>
              </View>
              <ScrollView contentContainerStyle={styles.content}>
                ...
                <View style={[styles.tipBox, { borderLeftColor: info.color }]}>
                  ...
                  <Text style={styles.tipText}>
                    テキストで基礎を固めたら科目別問題で知識を定着させ、模擬試験で仕上げましょう。学科試験は5科目とも一定の得点が必要になる形式が一般的なので、苦手科目を作らないことが鍵です。
                  </Text>
                </View>
              </ScrollView>
            </>
