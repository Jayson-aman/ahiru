...
const INFO: Record<string, { name: string; emoji: string; color: string }> = {
  ...
  setsubi: { name: '電気設備', emoji: '🏭', color: '#1565C0' },
  sekou: { name: '施工管理法', emoji: '🔧', color: '#2E7D32' },
  hoki: { name: '法規', emoji: '⚖️', color: '#6A1B9A' },
};

export default function DenkisekouTextScreen() {
  const { subject } = useLocalSearchParams<{ subject: string }>();
  const router = useRouter();
  const info = INFO[subject ?? ''] ?? { name: subject, emoji: '📖', color: '#EF6C00' };
  const sections = getDenkisekouText(subject ?? '');
