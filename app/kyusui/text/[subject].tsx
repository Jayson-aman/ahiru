...
export default function KyusuiTextScreen() {
  const { subject } = useLocalSearchParams<{ subject: string }>();
  const router = useRouter();
  const info = INFO[subject ?? ''] ?? { name: subject, emoji: '📖', color: '#0288D1' };
  const sections = getKyusuiText(subject ?? '');
              ...
              <ScrollView contentContainerStyle={styles.content}>
                ...
                <View style={[styles.tipBox, { borderLeftColor: info.color }]}>
                  ...
                  <Text style={styles.tipText}>
                    テキストで基礎を固めたら科目別問題で知識を定着させ、模擬試験で仕上げましょう。全科目に最低点があるため、苦手科目を作らないことが合格の鍵です。
                  </Text>
                </View>
              </ScrollView>
            </>
