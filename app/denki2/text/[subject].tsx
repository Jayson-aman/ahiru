...
const INFO: Record<string, { name: string; emoji: string; color: string }> = {
  riron:  { name: '電気の基礎理論',   emoji: '⚡', color: '#E65100' },
  haisen: { name: '配電理論と配線設計', emoji: '🔌', color: '#1565C0' },
  kiki:   { name: '機器・材料・工具',   emoji: '🔧', color: '#00695C' },
  hoki:   { name: '法令・配線図',       emoji: '📜', color: '#6A1B9A' },
};

export default function Denki2TextScreen() {
  const { subject } = useLocalSearchParams<{ subject: string }>();
  const router = useRouter();
  const info = INFO[subject ?? ''] ?? { name: subject, emoji: '📖', color: '#E65100' };
  const sections = getDenki2Text(subject ?? '');
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <CertPaywall
        certKey="denki2"
        certName={`第二種電気工事士 ${info.name} テキスト`}
              ...
              <ScrollView contentContainerStyle={styles.content}>
                ...
                <View style={[styles.tipBox, { borderLeftColor: info.color }]}>
                  ...
                  <Text style={styles.tipText}>
                    テキストで基礎を固めたら科目別問題で知識を定着させ、模擬試験で仕上げましょう。筆記は50問中30問（60%）で合格。複線図は筆記の配線図問題と技能試験の両方で必要なので、必ず手を動かして覚えましょう。
                  </Text>
                </View>
              </ScrollView>
            </>
