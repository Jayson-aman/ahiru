...
再表示はできないが、**失効に表示は不要**なので、履歴に残っているキーは
そのまま失効させればよい。

Stripe → 開発者 → APIキー → 該当キーの「…」→ 失効する

新しく作るときは**制限付きキー（`rk_live_`）**にする。アーカイブに必要な
権限は3つだけ:
...
漏れても商品の操作しかできないため、`sk_` と比べて被害が桁違いに小さい。
...
2. アーカイブを実行
3. そのキーを失効
4. 古い `sk_live_`（履歴に残っているもの）も失効

新しいキーの寿命を数分に抑える。あわせて Mac 側で:

```bash
unset STRIPE_SECRET_KEY
# ~/.zsh_history を開いて該当行を削除するのが確実
```
