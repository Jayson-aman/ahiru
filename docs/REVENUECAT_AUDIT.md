...
### 現状: Stripe Tax が「お客様のアカウントでは使用できません」

原因未特定。候補は Stripe アカウントの本人確認・事業情報が未完了
（売上 ¥0、事業設定を入力中だった）など。

**いまは RevenueCat 側の税計算を OFF のままにしておくのが正しい。**
`@revenuecat/purchases-js` に `StripeTaxNotActive` エラーがあり、
RevenueCat 側を ON にして Stripe 側が使えない状態だとチェックアウトが
エラー画面になる。関連エラー:
